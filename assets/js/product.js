(function ($) {
  ...
  const viewAlts = ['SELÇUK T-SHIRT tam görünüm', 'SELÇUK T-SHIRT göğüs arması detayı', 'SELÇUK T-SHIRT kol arması detayı'];
  ...
  function setView(index) {
    ...
    $('.gallery-main-image').removeClass('view-chest view-sleeve').toggleClass('view-chest', view === 'chest').toggleClass('view-sleeve', view === 'sleeve').attr('alt', viewAlts[currentView]);
  }
  ...
  function add() {
    ...
    const requested = Number($('#quantity').text()) || 1;
    const before = window.Shop.itemCount();
    window.Shop.addToCart(size, requested);
    const added = window.Shop.itemCount() - before;
    if (added < 1) {
      $message.text('Sepette en fazla 10 ürün olabilir.').addClass('is-visible').attr('role', 'alert');
    } else if (added < requested) {
      $message.text(`${added} ürün eklendi; sepet sınırı 10 adettir.`).addClass('is-visible').attr('role', 'status');
    } else {
  ...
  $(function () {
    ...
    $(document).on('keydown', (event) => { if (event.key === 'Escape') closeLightbox(); });
